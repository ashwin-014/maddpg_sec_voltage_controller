...
def build_summaries(n):
	...
	for k in range(n):
		...
		tf.summary.scalar("Reward_"+str(k), episode_reward[k])
...
#  ------------------- removed eval ------------------
			...
			for i in range(env.n):
				...
				if replayMemory.size()>int(args['minibatch_size']):
					...
					# logging.debug("s batch shape : {0}".format(s_batch.shape))
					# logging.debug("a batch shape : {0}".format(a_batch.shape))
					# logging.debug("r batch shape : {0}".format(r_batch.shape))
					# logging.debug("s2 batch shape : {0}".format(s2_batch.shape))
 ...
 # , axis=0
					...
					# logging.debug("s2_batch_i.shape : {0}".format( s2_batch_i.shape))
					...
					logging.debug("yi : {0}".format(np.array(yi).shape))
					# logging.debug(np.asarray([x.flatten() for x in a_batch]).shape)
					...
					critic.train(s_batch_i,np.asarray([x.flatten() for x in a_batch]),np.reshape(np.array(yi),(int(args['minibatch_size']),2))) # see if the value of 2 is correct
					# ------------------------------- changed --------------
