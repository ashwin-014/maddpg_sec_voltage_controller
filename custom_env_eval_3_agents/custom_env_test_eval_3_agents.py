...
MAIN_POWER_MAX_LIMIT = 7.2
MAIN_POWER_MIN_LIMIT = -7.2

SYS_POWER_MAX_LIMIT = 20
SYS_POWER_MIN_LIMIT = -20
...
# --------------------------------------------------------------------------------------------------------
    ...
    def adl_demand(self):
        x_w_batch = np.zeros(shape=(12,NUM_ADL,2))
    ...
    def non_adl_demand(self):
        ...
        probs = np.random.random(12)
        wx=np.rint(self.w_non_adl*(probs-0.2) + self.w_non_adl)
        wx/=max(wx)
    ...
    def remove(self, row_num,curr_step):
        ...
        if (self.allocated_load.size != 0):
            ...
            self.time_step_not_fully_allocated += NUM_AGENTS
            # --------------- print removed ----------------
...
class MultiAgent(gym.Env):
    ...
    def __init__(self):
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:                
                ...
                u_action_space = spaces.Box(low=SYS_POWER_MIN_LIMIT, high=SYS_POWER_MAX_LIMIT, shape=(2,)) # ,dtype=np.float32
    ...
    def step(self, action_n):
        ...
        for k,agent in enumerate(self.agents):
            ...
            reward_n.append(self._get_reward(agent, self.agents,action_n[k]))
    ...
    def find_sum(self, pow_set, action_load_alloc):
        ...
        for i in range(len(pow_set)):
            # print("diff[i]=",np.abs(np.sum(pow_set[i]) - 4*action_load_alloc[0]), "sum:", np.sum(pow_set[i]), "alloc:", 4*action_load_alloc[0])
            diff[i] = np.abs(np.sum(pow_set[i]) - NUM_ADL*action_load_alloc[0])
    ...
    def _get_reward(self,agent, agents, action):
        """Reward is given for energy_balance and all_allocated."""
        # for agent in agents:
        agent.cost=0
        ins = 0
        sum_allocated_load=0
        sum_ndt = 0

        if GRID_CONNECTED_MODE ==1:
            for i in range(NUM_AGENTS): 
                sum_allocated_load += agents[i].amount_allocated
                sum_ndt += agents[i].ndt

            if (((sum_allocated_load - sum_ndt)) <= MAIN_POWER_MAX_LIMIT) and (((sum_allocated_load - sum_ndt))>=MAIN_POWER_MIN_LIMIT):
                ins =10
                agent.cost += 1*agent.delay + self.unit_price[self.curr_step] * 10*(agent.amount_allocated - agent.ndt)
            else:
                agent.cost+=1000
            
            agent.rew += (10 + 10*ins - (agent.time_step_not_fully_allocated*20)  - (agent.cost) - np.abs(int(action[1]) - agent.allocated_load.shape[0])*50 - np.abs(action[0] - agent.amount_allocated)*50)
            ...
            if self.time_is_over:
                if not agent.failed_allocate:
                    ...
                    for k in range(NUM_ADL):
                        total_unalloc += len(agent.deq_for_unalloc[k])
                    agent.rew -= 80.0*total_unalloc
            ...
            if (((sum_allocated_load - sum_ndt)) <= 0) and (((sum_allocated_load - sum_ndt))>=0):
                ins =100
                agent.cost += 1*agent.delay + self.unit_price[self.curr_step] * 5*(agent.amount_allocated - agent.ndt)
            else:
                agent.cost+=5000
            
            agent.rew += (10 + 10*ins - (agent.time_step_not_fully_allocated*2)  - (agent.cost) - np.abs(int(action[1]) - agent.allocated_load.shape[0])*50 - np.abs(action[0] - agent.amount_allocated)*50)
    ...
    def _get_obs(self, agent):
        ...
        if not self.time_is_over:
            agent.state.append(self.unit_price[self.curr_step+1])
        else:
            agent.state.append(0)
        # if GRID_CONNECTED_MODE == 1:
        #     agent.state.append(1)
        # else:
        #     agent.state.append(0)
        agent.state.append(GRID_CONNECTED_MODE)
        agent.state.append(NUM_AGENTS)
        agent.state.append(NUM_ADL)
    ...
    def _get_initial_state(self):
        ...
        for i,agent in enumerate(self.agents):
            ...
            print("agent:", i, ": ADL :", agent.temp_adl)
            agent.temp_nadl = agent.non_adl_demand()
            print("agent:", i, ": Non-ADL :", agent.temp_nadl)
            agent.temp_bat = agent.battery_gen()
            print("agent:", i, ": Battery :", agent.temp_bat)
            agent.temp_ren = agent.renewable_gen()
            print("agent:", i, ": Ren :", agent.temp_ren)
            ...
            agent.state.append(self.unit_price[self.curr_step])
