...
# -----------------------------------------------------------------------------------------
    ...
    def concat_adl(self):
        ...
        for num in range(4):
            unalloc_in = False
            if np.all(self.deq_for_unalloc[num][0]):
                ...
                # code here
                inside = False
                for t in range(time-self.curr_step):
                    print("out",t,"deq",self.deq_adl[num][t][0])
                    if(self.deq_adl[num][t][0]) :
                        print("inside",self.deq_adl[num][t][0])
        ...
        if (self.allocated_load):
            for num in range(4):
                if not (np.any(np.all(self.ADL_list[num] == self.allocated_load, axis=1))):
                    ...
                    self.deq_for_unalloc[num].appendleft(self.ADL_list[num])                    
                else:
                    temp3.append((0,0))
                    temp4.append(0)
    ...
    def remove(self, row_num,curr_step):
        ...
        if (self.allocated_load.size != 0):
            for num in range(4):
                ...
                # if not(np.array(self.ADL_list[num]) in self.allocated_load.tolist() ) :
                    temp3.append(self.ADL_list[num])
                    temp4.append(self.ADL_list[num][0])  
                    ...
                    inside = False
                    if curr_step!=11 :
                        for t in range(1,(time-(curr_step))):
...
class MultiAgent(gym.Env):
    ...
    def _set_action(self, action, agent, action_space):
        ...
        agent.new_ADL_list_for_concat, agent.new_ADL_list_for_nn = agent.remove(np.argmax(action),self.curr_step)
    ...
    def _get_reward(self,agent):
        ...
        if (((agent.amount_allocated - agent.ndt)) <= MAIN_POWER_MAX_LIMIT) and (((agent.amount_allocated - agent.ndt))>=MAIN_POWER_MIN_LIMIT):
            agent.cost += 1*agent.delay + self.unit_price * (agent.amount_allocated - agent.ndt)
        else:
            agent.cost+=101
        
        agent.rew += (1200 - (agent.time_step_not_fully_allocated*100)  - (agent.cost/10))
        ...
        if self.time_is_over:
            if not agent.failed_allocate:
                ...
                agent.rew -= 1500.0*(len(agent.deq_for_unalloc[0])+len(agent.deq_for_unalloc[1])+len(agent.deq_for_unalloc[2])+len(agent.deq_for_unalloc[3]))
